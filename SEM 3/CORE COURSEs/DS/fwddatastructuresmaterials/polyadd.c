#include <stdio.h>
#include <stdlib.h>
//Program to add 2 polynomials represented as SLLL
typedef struct abb
{
    int coeff,expn;
    struct abb *link;
...
term *create_poly(term *p1)
{
    ...
    while (choice)
    {
        printf("Input the Coeffecient and Exponeniation\n");
        scanf("%d%d",&coeff,&expn);
        p1 = insert_inorder(p1, coeff,expn);
        printf("Input 1 to continue 0 to Exit");
        scanf("%d",&choice);
    }//end of while loop
    return p1;
...
int main()
{
    ...
    printf("PRINTING 1st POLYNOMIAL\n");
    display(poly1);
