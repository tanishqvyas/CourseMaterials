...
int main() 
{
	...
	for (int i = 0; i < numOfJobs; ++i)
	{
		...
		scanf("%d", &burstTime[i]);
		printf("Enter Priority for JOB (%d) : ", jobID[i]);
	...
	for (int i = 0; i < numOfJobs; ++i)
	{
		...
		for (int k = 0; k < numOfJobs; ++k)
		{
			if(isScheduled[k] == 0)
			{
				if(jobPriority[k] < jobPriority[idx])
				{
					idx = k;
				}

				else if(jobPriority[k] == jobPriority[idx])
				{
